...
/*
 Allows the function to be called witout specifying an external paramater name.
 */
/*:
...
/*
 Offsets is an array of the positions of the 8 cells around a particular cell.
 */
/*:
 ## Problem 6:
...
/*
 You would use "of" before an argument when calling the "neighbors" function.
 */
/*:
...
/*
 You would use "cell" inside of the function when refering to the argument that was passed to "of".
 */
...
extension Grid {
    ...
    func neighbors(of cell: Cell) -> [Position] {
        return Grid.offsets.map {
            ...
            let row = (cell.position.row + $0.row + self.rows) % self.rows
            let col = (cell.position.col + $0.col + self.cols) % self.cols
            return (row, col)
        }
    }
}
...
/*
 Find the total number of something in row and col
 */
/*:
...
/*
 There is no generic T parameter because reduce2 can only be used for Int.
 */
...
/*
 Probability is not guaranteed to be exact — if we tossed a three-sided die 100 times, it's not guaranteed to land on 2 exactly 33 times.
 */
/*:
 ## Problem 14:
...
extension Grid {
    subscript (row: Int, col: Int) -> Cell? {
        ...
        set {
            ...
            guard row >= 0 && row < rows && col >= 0 && col < cols, let newValue = newValue else { return }
...
/*
 We don't know if a Cell exists at [row,col], so it needs to be an optional.
 */
/*:
...
/*
 The else clause will be executed when row or col are outside of the grid.
 */
/*:
 ## Problem 16:
...
/*
 The reduce function returns the number of living neighbors of a cell.
 */
...
/*
 A shorthand argument name for the position of the cell at [row, col]
 */
...
/*
 "nextGrid" is the entire grid for the next step in Conway's Game of Life.
 */
/*:
 ## Problem 22:
